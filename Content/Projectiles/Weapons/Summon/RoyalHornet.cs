			...
			if (Projectile.ai[1] == 0f)
			{
				...
				if (Main.myPlayer == Projectile.owner)
				{
					for (int i = 0; i < 3; i++)
					{
						...
						int newProjRet = Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, newProjectileSpeed.RotatedBy((i - 1) * 0.0872), new_projectile_type, Projectile.damage, Projectile.knockBack, Main.myPlayer);
